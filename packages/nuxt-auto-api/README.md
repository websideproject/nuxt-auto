...
- **Server-owned columns** — primary keys, tenant, soft-delete and audit columns can never be written by a request body
...
| Request bodies | Primary key, tenant, soft-delete and audit columns are dropped; field-level `write` rules refuse with 403 |
