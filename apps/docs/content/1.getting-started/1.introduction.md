...
- Deny-by-default authorization (operation, row, object and field level)
...
// …and you declare who may do what (anything undeclared is refused)
export const usersAuth: ResourceAuthConfig = {
  permissions: { read: ctx => !!ctx.user, update: 'admin', create: 'admin', delete: 'admin' },
}
```

Validation (from the schema), row filtering, pagination and the endpoints themselves come for free.
