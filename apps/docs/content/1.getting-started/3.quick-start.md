  ...
  setup(_options, nuxt) {
    ...
    nuxt.hook('autoApi:registerSchema', (registry) => {
      registry.register('posts', {
        ...
        authorization: createModuleImport(resolver.resolve('./auth'), 'openAuth'),
      })
...
Create `modules/base/auth.ts`. Every operation must be allowed explicitly — a resource without
`authorization` refuses every request:

```typescript
import type { ResourceAuthConfig } from '@websideproject/nuxt-auto-api'

// Open to everyone — fine for this local walkthrough, never for production.
// See /auto-api/authentication-authorization for users, roles and ownership rules.
export const openAuth: ResourceAuthConfig = {
...
- **[Authorization](/auto-api/authentication-authorization)** - Secure your endpoints
...
- **[Multi-Tenancy](/auto-api/multi-tenancy)** - Build SaaS applications
