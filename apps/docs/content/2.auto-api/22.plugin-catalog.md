...
Protects API endpoints from abuse with a pluggable backing store **or** a decision-based limiter.

- **Hooks used:** `pre-auth` middleware (runs after context extenders → `ctx.user` / `ctx.requestMeta.billing` available)
- **Backends:**
  - `store` — counter-based (`InMemoryRateLimitStore` default; supply Cloudflare KV / Redis for multi-isolate). Gives full `X-RateLimit-*` headers.
  - `limiter` — decision-based; shape matches the **Cloudflare Workers RateLimit binding** exactly, so `env.RATE_LIMITER` is passed straight through (native distributed counting, no Durable Object). Takes precedence over `store`.
- **Extras:** `Retry-After` on 429, `failOpen` (default `true`). Keys on `CF-Connecting-IP` only on Cloudflare, and on `X-Forwarded-For` only with `trustProxy` — elsewhere a client could set them and dodge the limit.
- **Use case:** Brute-force/DDoS protection, per-plan API quotas (pick the binding by `ctx.requestMeta.billing`).

```typescript
// counter store (default in-memory; KV for Workers)
createRateLimitPlugin({ windowMs: 60000, max: 100, byIp: true })

// Cloudflare RateLimit binding, plan-aware (free vs paid)
...
See [Rate Limiting](/auto-api/rate-limiting) for the store/limiter interfaces and Cloudflare setup.
...
- **Approach:** Reads the connection address and, on Cloudflare only, its `CF-*` headers (`X-Forwarded-For` with `trustProxy`), populates `context.requestMeta`. Optional before-hooks write metadata to DB columns via data return value.
...
- **Reading it:** the audit table is a registered resource, and `GET /api/audit-logs` is its list — through that resource's own authorization (`read`, tenant, `listFilter`, hidden fields), newest first (ties broken by the key). `?resource=`, `?recordId=` and `?userId=` are shorthands for `filter`; `limit` / `page` / `cursor` page as on any list. Adds a route, so list it in `nuxt.config` (see [Plugin Registration](/auto-api/plugin-system#plugin-registration)).
...
- **Reading it:** `GET /api/activities` is the activity table's list, through that resource's own authorization — the same as the audit-log route above. Adds a route, so list it in `nuxt.config` (see [Plugin Registration](/auto-api/plugin-system#plugin-registration)).
...
Adds `GET /api/{resource}/export?format=csv|json` to every resource (or those in `resources`).

- **Hooks used:** `buildSetup` (one route per resource)
- **Approach:** An export is the resource's list, page after page, up to `maxRows`: the same caller, tenant, `read` permission, row visibility, hidden and readable fields, and the same `filter` / `sort` / `fields` parameters. CSV cells that a spreadsheet would run as formulas (`=`, `+`, `-`, `@`) are prefixed with `'`. Adds a route, so list it in `nuxt.config` (see [Plugin Registration](/auto-api/plugin-system#plugin-registration)).
...
- **Hooks used:** `buildSetup` (`POST` / `DELETE /api/{resource}/:id/upload` for each configured resource)
- **Approach:** An upload is an update of the record: the caller needs `update` on the resource, the row must be visible to them (tenant, `listFilter`, `objectLevel`) and they must be allowed to write the file column. The file is stored under a server-generated name (local filesystem or NuxtHub Blob) and its URL saved in the column; `DELETE` clears it and only ever removes a file inside the upload directory. `accept` is checked against the type the client declares — do not serve uploads from your app's origin as HTML/SVG. Adds a route, so list it in `nuxt.config` (see [Plugin Registration](/auto-api/plugin-system#plugin-registration)).
...
- **Hooks used:** `afterCreate` / `afterUpdate` / `afterDelete` (write a snapshot of the committed state), `beforeDelete` (stash the row)
- **Approach:** Every write stores a numbered snapshot (sensitive fields stripped via `excludeFields`), pruned to `maxRevisionsPerRecord`. The plugin adds **no routes**: the revisions table is a registered resource — read it with `GET /api/revisions?filter=…` under its own authorization; restoring a version is an update of the record with that snapshot.
...
Caching for list/get operations with automatic invalidation on mutations, via a pluggable store.

- **Hooks used:** `pre-execute` middleware (serve from cache via `context.shortCircuit`), `post-execute` middleware (store the response — `data` and `meta`, so cursors and totals survive — and, after any successful write to the resource, including bulk, restore and M2M, drop its entries)
- **Keys:** resource, operation, path, query, user and tenant by default (`keyGenerator` to change). Only the written resource is invalidated: a cached list that `?include=`s another resource can show that resource's older rows for up to `ttlMs`.
- **Backend:** `store` — `InMemoryCacheStore` default (TTL + LRU eviction + per-prefix invalidation). Supply a `CacheStore` over **Cloudflare KV** for cross-isolate caching/invalidation on Workers.
...
createCachePlugin({
  ...
  // store: new KvCacheStore(env.CACHE_KV)   // on Cloudflare Workers
})
```

> **Cloudflare caveat:** the default in-memory store is per-isolate — low hit-rate and no cross-isolate invalidation on Workers. Pass a KV-backed `CacheStore` there. Tenant-scoped resources must include the auth scope in the cache key (`keyGenerator`) or set `resources` to public read-mostly tables only.
...
1. **Token CRUD lifecycle** — hooks on your token resource(s) handle hashing on create, masking on read, rotation on update, and cache invalidation on delete. A token always belongs to the caller who creates it (and, with `orgField`, their active organization): an owner or organization in the request body is ignored, and an update cannot move a token to someone else.
...
The owner (`userId`) is always the caller (`context.user.id`) and, for team tokens, `orgField` is always their active organization (`context.tenant.id`) — values sent in the body are ignored.
...
Cache entries expire after `ttlMs` (default 5 min), checked on lookup — there is no timer (Cloudflare Workers do not allow one at startup). When max entries is reached, expired entries are swept and then the oldest entry is evicted (Map insertion order). Delete and rotate operations invalidate the cache immediately via the reverse map.
...
| Minting for someone else | The owner (`userRelation.field`) and organization (`orgField`) are set from the caller on create and cannot be changed by an update. |
...
![The playground's scope test with the editor token, which holds articles:read, articles:create and articles:update: GET /api/articles is allowed; GET /api/posts, GET /api/users and DELETE /api/articles/1 are denied](/screenshots/api-token-scopes-1440.png)
...
| Revision History | Shipped | before/after hooks |
