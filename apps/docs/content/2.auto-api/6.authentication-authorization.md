...
### What field permissions actually do

**`read` filters; `write` refuses.** They are asymmetric on purpose:

- a field the caller may not **read** is *removed from the response* (list, get, create, update, bulk).
  Refusing the request instead would 403 an ordinary list for an ordinary user, which is not what
  "you cannot see this column" means.
- a field the caller may not **write** makes the request fail with **403, naming the fields**, and nothing
  is written. A body mixing an allowed field with a denied one changes *nothing* — it does not half-apply.
  Silently dropping the field would tell a caller its edit succeeded when it did not.

Three things worth knowing:

- **The write gate reads the REQUEST BODY, before hooks run.** `beforeCreate`/`beforeUpdate` hooks write
  fields on the server's own behalf (tenant injection, derived totals); the gate is about what the *caller*
  asked for, not what the server adds.
- **A declaration with only `read` does not restrict writing**, and vice versa. `{ read: isAdmin }` says
  nothing about writes.
- ⚠ **The `*` permission wildcard does NOT open a field gate.** Operation-level permissions short-circuit
  on `*`; field gates evaluate the predicate, so `read: () => false` holds even for a platform admin — and
  `GET /api/<resource>/permissions` reports exactly what will be enforced.

::callout{icon="i-lucide-triangle-alert"}
**Root resource only.** Columns reached through `?include=` belong to the nested resource, and the registry
cannot map a relation key back to a resource name. A denied column on an included relation is still
returned — use `hiddenFields` for that case.
::

::callout{icon="i-lucide-info"}
The generated admin panel reads `fields[x].canWrite` from `/permissions` and renders non-writable columns
read-only, so a form never submits a field the API will refuse.
::

## Soft-delete permissions (restore / purge / viewDeleted)

When a resource is soft-deletable (see [Soft Deletes](./5.soft-deletes)), three extra permission keys
gate the recovery/destructive operations. They are **first-class** — resolved by the same engine as
`read`/`create`/`update`/`delete`, so a role string, `string[]`, function, or entitlement-gate object
all work. There's also a typed `softDelete{}` block as an alternative home (and for `cascade`/retention).
...
export const articlesAuth: ResourceAuthConfig = {
  permissions: {
    ...
    viewDeleted: 'editor',      // ?includeDeleted / ?onlyDeleted in list/get/aggregate
    // or an entitlement gate: restore: { gate: 'trash:restore' }
  },
  // Equivalent home (permissions.* wins when both are set):
  softDelete: {
    restore: 'editor',
    purge: 'admin',
    viewDeleted: 'editor',
    cascade: 'auto',            // 'auto' (default) mirrors FK onDelete; 'off' disables
    retentionDays: 30,          // trash purge window (0 = never)
  },
}
```

**Resolution / safe defaults** (never fail-open):
