...
export default defineNitroPlugin(() => {
  addResourceHook('users', {
    afterCreate: async (result, context) => {
      ...
      console.log(`New user from ${country}: ${context.requestMeta?.ip}`)
    },
  })
})
```
...
Custom function to extract metadata from the request. The default reads Cloudflare's headers when the request came through Cloudflare, and the connection's address otherwise.
...
- IP: `CF-Connecting-IP` on Cloudflare; the `X-Forwarded-For` entry your proxy saw with `trustProxy`; otherwise
  the connection's address
- Geo (on Cloudflare only — elsewhere a client could send these headers):
...
Behind your own reverse proxy, how many proxies append to `X-Forwarded-For` (`true` = 1). Without it the header
is ignored: a client can write any address there.
...
// Hook to also update timestamp (server/plugins/user-hooks.ts)
addResourceHook('users', {
