...
Bulk delete, CSV/JSON export and CSV import are built in — see
[Features](/auto-admin/configuration-theming#features).
...
actions: {
  exportAll: {
    ...
    handler: async (_, context) => {
      const response = await $fetch('/api/posts/export', { query: { format: 'csv' } })
      // Download file logic
