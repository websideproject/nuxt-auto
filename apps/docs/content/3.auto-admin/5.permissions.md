...
- The list toolbar: "Delete selected" (`canDelete`), "Export" (`canRead`) and "Import" (`canCreate`) — see
  [Features](/auto-admin/configuration-theming#features). Row checkboxes appear only with `canDelete`.
- Action buttons in table context menus (always shown but disabled)

Edit and Delete — on the detail page, the edit page, the view dialog, each row's menu and each row's checkbox —
follow **that record's** permission, not just the resource's: a resource with an `objectLevel` rule (only the
author may edit, drafts are read-only) gets them disabled on exactly the rows the API would refuse. The admin
asks `GET /api/{resource}/permissions?ids=…` once per page of rows.
...
![The articles list as a regular user: drafts are not listed, Create New and Delete selected are disabled, rows have no checkboxes, and the row menu offers View while Edit and Delete are disabled](/screenshots/admin-list-user-1440.png)
...
customPages: [
  {
    ...
    // '<resource>:<action>' — what GET /api/permissions says the caller may do
    permissions: 'settings:update'
    // OR several (the caller needs ALL)
    // permissions: ['settings:update', 'users:read']
  }
]
```

Actions are `create`, `read`, `update`, `delete`, `restore` and `purge`. An entry in any other form grants
nothing, so a typo hides the page rather than opening it. For any other rule, give the page itself a route
middleware (`definePageMeta({ middleware: … })`); a `canAccess` function in `nuxt.config` could never reach the
running app and now fails the build.
...
### Components
- `ResourceTable.vue` - Context menu actions always shown but disabled; toolbar buttons follow `unauthorizedButtons`
- `ResourceAuditLog.vue` - History panel; a caller who may not read the audit log gets a note, not an error
...
2. **Custom pages use `permissions: ['<resource>:<action>']`**, or their own route middleware for other rules
